...
def reportBuildFailure() {
    ...
    sh 'GITHUB_TOKEN="$GITHUB_CREDENTIAL_PSW" bash ci/report-build-failure.sh || true'
}
...
pipeline {
    ...
    stages {
        ...
        stage('Deliver Docker images for PR') {
          ...
          steps {
            tee('ci-logs/Deliver Docker images for PR.log') {
              script {
                if (env.CHANGE_FORK) {
        ...
        stage('Deliver Docker images') {
          ...
          steps {
            tee('ci-logs/Deliver Docker images.log') {
              script {
