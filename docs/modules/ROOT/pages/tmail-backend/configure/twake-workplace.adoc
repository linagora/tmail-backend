...
=== HasAnySaaSPlan matcher

The `HasAnySaaSPlan` matcher can be used to determine if the sender of an email has a SaaS plan.

=== HasSaaSPlan matcher

The `HasSaaSPlan` matcher allows to match emails where the sender has a specific SaaS plan.
...
[source,xml]
----
<mailet match="HasSaaSPlan=standard,premium" class="..." />
----
