...
JMAP settings are user-level key/value preferences stored server-side and accessible via the JMAP protocol. Each key must be non-empty, at most 255 characters, and composed only of alphanumeric characters plus `.`, `-`, `_`, and `#`. Values are plain strings.

=== Get JMAP settings of a user

....
curl -XGET http://ip:port/users/user@domain.tld/jmap/settings
....

Returns the user's settings as a flat JSON object:
...
An empty object `{}` is returned when the user has no stored settings.

Return codes:

- `200` The settings were successfully retrieved
- `404` The user does not exist

=== Set JMAP settings of a user

....
curl -XPUT http://ip:port/users/user@domain.tld/jmap/settings \
  -H "Content-Type: application/json" \
  -d '{"language": "fr", "trash.cleanup.enabled": "true"}'
....

Fully replaces the user's settings with the provided map. Omitted keys are removed. Pass `{}` to clear all settings.

Return codes:

- `204` The settings were successfully stored
- `400` One or more keys are invalid (too long, empty, or contain disallowed characters)
- `404` The user does not exist
...
A task is considered to belong to a domain when its additional information references a user of that domain, or the domain itself. If the task exists but does not belong to the requested domain, a `404` is returned — this is intentional and avoids leaking information about tasks belonging to other domains.

=== Listing the tasks of a domain

....
curl -XGET http://ip:port/domains/domain.tld/tasks
....

Will return the list of execution reports of the tasks belonging to the domain, in the same format as `GET /tasks`,
sorted by reverse submitted date (recent tasks goes first). Tasks that do not belong to the domain are never returned.

One can filter the above results by status. For example:

....
curl -XGET http://ip:port/domains/domain.tld/tasks?status=inProgress
....

`status` is one of `waiting`, `inProgress`, `canceledRequested`, `completed`, `canceled`, `failed`.

Response codes:

- `200` A list of the tasks belonging to the domain is returned (possibly empty)
- `400` Invalid status value
- `404` The domain does not exist

=== Getting a domain-scoped task details

....
curl -XGET http://ip:port/domains/domain.tld/tasks/3294a976-ce63-491e-bd52-1b6f465ed7a2
....

Response codes:

- `200` The task belongs to the domain; its execution report is returned
- `400` Invalid task ID
- `404` The domain does not exist, the task ID was not found, or the task does not belong to the domain

=== Awaiting a domain-scoped task

....
curl -XGET http://ip:port/domains/domain.tld/tasks/3294a976-ce63-491e-bd52-1b6f465ed7a2/await
....

Response codes:

- `200` The task belongs to the domain and completed (successfully or not)
- `400` Invalid task ID
- `404` The domain does not exist, the task ID was not found, or the task does not belong to the domain

=== Cancelling a domain-scoped task

....
curl -XDELETE http://ip:port/domains/domain.tld/tasks/3294a976-ce63-491e-bd52-1b6f465ed7a2
....

Response codes:

- `204` Cancellation request was accepted
- `400` Invalid task ID
- `404` The domain does not exist, the task ID was not found, or the task does not belong to the domain
...
Exposes the total current mailbox quota usage (message count and storage size), aggregated across all users or restricted to a single domain.

The `count` field is the total number of messages, and the `size` field is the total storage in bytes. Only mailbox quota rows (`QuotaComponent.MAILBOX`) are accounted for; other quota components such as Sieve are excluded.

=== Get the global quota usage sum

....
curl -XGET http://ip:port/quota/sum
....

Returns the sum of current mailbox quota usage across all users of the server:
...
- `200` the aggregated sum was returned

=== Get the quota usage sum for a domain

....
curl -XGET http://ip:port/quota/domains/:domain
....

Returns the sum of current mailbox quota usage for the users of the given domain. The users of the domain are enumerated and their individual mailbox quotas are summed, which is more efficient than scanning all quota rows in multi-tenant (SaaS) deployments with many small domains.
...
- `200` the aggregated sum was returned (returns `{ "count": 0, "size": 0 }` when the domain has no usage or no users)
...
== JMAP Settings reports

Returns an aggregated view of all JMAP settings values across users, useful for auditing configuration adoption. The response is a two-level map: settings key → settings value → number of users with that combination.

=== Get JMAP settings usage report

....
curl -XGET http://ip:port/jmap/settings/reports
....

Optionally restrict to a single domain:

....
curl -XGET "http://ip:port/jmap/settings/reports?domain=linagora.com"
....
...
Users with no stored settings are omitted from the counts. Keys and values that no user has set do not appear in the response.

Return codes:

- `200` The report was successfully generated
...
=== Applying domain signature templates to existing users

....
curl -XPOST http://ip:port/domains/domain.tld/signature-templates?action=apply
....

Applies the domain signature template to all users of the domain who do not yet have a signature
on their default JMAP identity. Users who already have a non-empty signature are skipped.

LDAP attribute placeholders (e.g. `{ldap:givenName}`) are resolved at apply time using the same
LDAP connection used by `IdentityProvisionListener`.

Optional query parameters:

- `overwriteExistingSignatures` — when present, overwrites existing signatures instead of skipping users who already have one.

....
curl -XPOST http://ip:port/domains/domain.tld/signature-templates?action=apply&overwriteExistingSignatures
....
...
| applied | Number of users whose signature was set
| skipped | Number of users skipped (no stored identity, or signature already present and overwrite not requested, or no template matched their locale)
| error   | Number of users for whom the operation failed (details logged server-side)
|===

Return codes:

- `200` Operation completed; see body for counts
- `404` Domain does not exist, or has no stored signature template
- `400` Unknown `action` query parameter
