...
- *validUntil*: `UTCDate`. Server-set. The instant until which the token grants access, as an RFC 3339 UTC date
  (for example `2026-08-28T12:00:00Z`). Matches the `tokenTtlInSeconds` advertised by the capability, counted from
    ...
    [
      "UnauthenticatedBlobAccess/set",
      {
        ...
        "created": {
          "BLOB_ID_1": {
            "token": "TOKEN_VALUE",
            "validUntil": "2026-08-28T12:05:00Z"
          }
        }
      },
      "c1"
