...
For compatibility with older mobile clients, the legacy setting key `ai.needs-action.enabled`
is still supported. The system prioritizes the new setting if both are present:

1. If `ai.label-categorization.enabled` exists → uses this value
2. Otherwise, if `ai.needs-action.enabled` exists → uses this value (legacy)
3. Otherwise → uses default value (`false`)
