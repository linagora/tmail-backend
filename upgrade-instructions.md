...
### Adding documentation field to labels table
...
Add the optional `documentation` field to the `labels` table to allow users to provide clear descriptions for their labels. This enables better frontend synchronization.
...
```cql
ALTER TABLE labels ADD documentation text;
```
#### Postgres
To add this column, run the following SQL command:

```sql
ALTER TABLE labels ADD COLUMN documentation VARCHAR;
```
