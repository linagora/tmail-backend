...
## New upgrade instructions location

Starting from version 1.0.16, upgrade instructions are embedded directly in the Docker images and maintained
per-application in this repository:

- **Distributed (Cassandra):** [tmail-backend/apps/distributed/src/doc/upgrade-instructions](tmail-backend/apps/distributed/src/doc/upgrade-instructions)
- **PostgreSQL:** [tmail-backend/apps/postgres/src/doc/upgrade-instructions](tmail-backend/apps/postgres/src/doc/upgrade-instructions)

Each version has its own `index.md` file. Instructions are also available inside the running container at
`/root/doc/upgrade-instructions/<version>/index.md`.

This file will no longer be updated for new releases.
