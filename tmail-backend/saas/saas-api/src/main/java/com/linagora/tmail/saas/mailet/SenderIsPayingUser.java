...
/**
 * Matches mail where the sender is a paying SaaS user.
 */
public class SenderIsPayingUser extends GenericMatcher {
    private final SaaSAccountRepository saaSAccountRepository;

    @Inject
    public SenderIsPayingUser(SaaSAccountRepository saaSAccountRepository) {
    ...
    public final Collection<MailAddress> match(Mail mail) {
        return Mono.justOrEmpty(mail.getMaybeSender().asOptional())
            ...
            .flatMap(sender -> Mono.from(saaSAccountRepository.getSaaSAccount(sender))
                .map(SaaSAccount::saaSPlan)
                .filter(saasPlan -> !saasPlan.equals(SaaSPlan.FREE))
                .map(payingPlan -> mail.getRecipients()))
