...
/**
 * A deployment whose Kerberos realm is not its mail domain declares the correspondence in
 * {@code kerberos.realmMapping}: the principal the KDC authenticates is then not the identity the
 * backend session is opened for, which is what this covers. {@link MigrationGssapiTest} covers the
 * default, where the realm is the domain.
 */
...
class MigrationGssapiRealmMappingTest {
    private static final String SERVICE = "imap";
    // The KDC fixture issues tickets for alice@JAMES.TEST, which the mapping below turns into a user of
    // the mail domain - a domain the Kerberos realm bears no relation to.
    private static final String MAIL_DOMAIN = "domain.tld";
    ...
    static void setUpAll() throws Exception {
        ...
        // conf/migrationproxy.properties keys this onto the KerberosTestFixture.REALM realm.
        setProperty("migration.kerberos.realmMapping", MAIL_DOMAIN);
    ...
    void gssapiShouldOpenTheBackendSessionForTheMappedUser() throws Exception {
        try (ProxyImapClient client = new ProxyImapClient(PROXY_IMAP_PORT);
            ...
            // alice@JAMES.TEST authenticated, alice@domain.tld is the identity the backend serves.
