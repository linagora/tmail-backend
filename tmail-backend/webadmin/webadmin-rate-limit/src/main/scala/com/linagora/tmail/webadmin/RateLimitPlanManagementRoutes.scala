...
class RateLimitPlanManagementRoutes @Inject()(planRepository: RateLimitationPlanRepository,
  ...
  private val BASE_PATH = SEPARATOR + "rate-limit-plans"
  private val CREATE_A_PLAN_PATH = BASE_PATH + SEPARATOR + PLAN_NAME_PARAM
  private val UPDATE_A_PLAN_PATH = BASE_PATH + SEPARATOR + PLAN_ID_PARAM
  private val GET_A_PLAN_PATH = BASE_PATH + SEPARATOR + PLAN_ID_PARAM
  ...
  private def toRateLimitationDTO(rateLimitation: RateLimitation): RateLimitationDTO =
    new RateLimitationDTO(rateLimitation.name, toPeriodString(rateLimitation.period),
      extractLimitType(rateLimitation, LimitTypes.COUNT), extractLimitType(rateLimitation, LimitTypes.SIZE))

  private def toPeriodString(duration: Duration): String = String.format("%s second", duration.getSeconds)
  ...
  private def toRateLimitation(dto: RateLimitationDTO): RateLimitation =
    RateLimitation(name = dto.getName, period = toDuration(dto.getPeriod), limits = toLimitTypes(dto.getCount, dto.getSize))

  private def toDuration(period: String): Duration =
    DurationParser.parse(period, ChronoUnit.SECONDS)
