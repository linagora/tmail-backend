...
import com.linagora.tmail.james.common.LinagoraCalendarEventRejectMethodContract.PASSWORD
...
object LinagoraCalendarEventRejectMethodContract {
  val PASSWORD = "1"
}
...
abstract class LinagoraCalendarEventRejectMethodContract {
  ...
  def shouldReturnNotFoundResultWhenBlobIdDoesNotExist(server: GuiceJamesServer): Unit = {
    val notFoundBlobId =
      sendDynamicInvitationEmailAndGetIcsBlobIds(
        server, "template/emailWithAliceInviteBobIcsAttachment.eml.mustache", sender, receiver, icsPartId = "999999")
