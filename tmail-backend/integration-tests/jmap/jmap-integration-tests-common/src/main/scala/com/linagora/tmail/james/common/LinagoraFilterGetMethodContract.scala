...
trait LinagoraFilterGetMethodContract {
  ...
  @Test
  def filterCapabilityShouldReturnVersion(): Unit = {
    `given`()
    .when()
    ...
    .`then`
      .statusCode(SC_OK)
      .contentType(JSON)
      .body("capabilities.'com:linagora:params:jmap:filter'", hasKey("version"))
      .body("capabilities.'com:linagora:params:jmap:filter'.version", equalTo(2))
  }
