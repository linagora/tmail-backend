...
trait TeamMailboxMemberSetMethodContract {
  ...
  @Test
  def updateShouldReturnNotUpdatedWhenTeamlMailboxNameIsInvalid(server: GuiceJamesServer): Unit = {
    val teamMailbox = TeamMailbox(DOMAIN, TeamMailboxName("hiring"))
    ...
    `given`
      ...
      .body("methodResponses[0]", jsonEquals(
        s"""[
           ...
           |            "${teamMailbox.asString()}": {
           |                "type": "invalidPatch",
           |                "description": "Wrong teamMailboxName ${teamMailbox.asString()}"
           |            }
           |        }
           |    },
           |    "c1"
    ...
    `given`
      ...
      .body("methodResponses[0]", jsonEquals(
        s"""[
           ...
           |            "${teamMailbox2.asString()}": {
           |                "type": "invalidPatch",
           |                "description": "Wrong teamMailboxName ${teamMailbox2.asString()}"
           |            }
           |        }
           |    },
           |    "c1"
