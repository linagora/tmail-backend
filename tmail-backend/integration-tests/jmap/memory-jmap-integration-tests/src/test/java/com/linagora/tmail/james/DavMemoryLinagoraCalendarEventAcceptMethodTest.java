...
public class DavMemoryLinagoraCalendarEventAcceptMethodTest implements
    LinagoraCalendarEventAcceptMethodContract {
        ...
        return new AbstractModule() {
            ...
            public OpenPaasConfiguration provideOpenPaasConfiguration(DavConfiguration davConfiguration)
                ...
                return new OpenPaasConfiguration(
                    URI.create("http://%s:8080/api".formatted(dockerOpenPaasExtension.getOpenPaasServerIpAddress())),
                    "admin@open-paas.org",
                    "secret",
                    true,
                    Optional.of(new OpenPaasConfiguration.ContactConsumerConfiguration(
                        ImmutableList.of(AmqpUri.from("amqp://%s:5672".formatted(dockerOpenPaasExtension.getRabbitMqIpAddress()))),
    ...
    @Override
    public InvitationEmailData constructEmailInvitation() {
        OpenPaasUser sender = dockerOpenPaasExtension.newTestUser();
        OpenPaasUser receiver = dockerOpenPaasExtension.newTestUser();

        return new InvitationEmailData(
            new User(sender.firstname(), Username.of(sender.email()), sender.password()),
            new User(receiver.firstname(), Username.of(receiver.email()), receiver.password()));
    }
}