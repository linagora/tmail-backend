...
class DistributedTeamMailboxDeletedMessageVaultIntegrationTest {
    ...
    private String teamMailboxAddress;
    ...
    void setUp(GuiceJamesServer server) throws Exception {
        ...
        andre = context.andre;
        bob = context.bob;
        teamMailbox = context.teamMailbox;
        teamMailboxAddress = context.teamMailboxAddress;
    ...
    void restoreTaskShouldRestoreVaultedMessagesToTeamMailboxRestoreFolder(GuiceJamesServer server) throws Exception {
        ...
        WAIT_AT_MOST_ONE_MINUTE
            .until(() -> Flux.from(vaultProbe.getVault().search(teamMailbox.self(), Query.ALL))
                .count()
                .block() > 0);
        ...
        .then()
            ...
            .body("additionalInformation.teamMailboxAddress", is(teamMailboxAddress))
            .body("additionalInformation.successfulRestoreCount", is(1))
            .body("additionalInformation.errorRestoreCount", is(0));

        MailboxPath restorePath = teamMailbox.mailboxPath(VaultConfiguration.DEFAULT.getRestoreLocation());
    ...
    void expungeMessageShouldAppendToTeamMailboxVaultWhenMemberStillHasAnotherReference(GuiceJamesServer server) throws Exception {
        ...
        ComposedMessageId teamMessage = appendMessage(mailboxProbe, teamMailbox.inboxPath(), MESSAGE_CONTENT);
        mailboxProbe.copy(bob, teamMailbox.inboxPath(), MailboxPath.forUser(bob, PERSONAL_BOX), teamMessage.getUid());

        // WHEN the member expunges the message from the team mailbox
        mailboxProbe.deleteMessage(ImmutableList.of(teamMessage.getUid()), teamMailbox.inboxPath(), bob);

        // THEN the message is appended to the team mailbox vault and not to the member personal vault
        awaitVaultCount(vaultProbe, teamMailbox.self(), 1);
    ...
    void mailboxDeletionShouldAppendDeletedMessagesToTeamMailboxVault(GuiceJamesServer server) throws Exception {
        MailboxProbeImpl mailboxProbe = server.getProbe(MailboxProbeImpl.class);
        TmailBlobStoreDeletedMessageVaultProbe vaultProbe = server.getProbe(TmailBlobStoreDeletedMessageVaultProbe.class);
        MailboxPath teamFolderPath = teamMailbox.mailboxPath(TEAM_FOLDER);
        ...
        awaitVaultCount(vaultProbe, teamMailbox.self(), 2);
    }
    ...
    void personalDeleteShouldNotAppendToMemberVaultWhileTeamMailboxReferenceStillExists(GuiceJamesServer server) throws Exception {
        ...
        // AND a team mailbox message copied into the member personal mailbox
        ComposedMessageId teamMessage = appendMessage(mailboxProbe, teamMailbox.inboxPath(), MESSAGE_CONTENT);
        mailboxProbe.copy(bob, teamMailbox.inboxPath(), personalBox, teamMessage.getUid());
        ...
        awaitVaultCount(vaultProbe, teamMailbox.self(), 0);
    }
    ...
    void personalDeleteShouldAppendToMemberVaultWhenTeamMailboxReferenceNoLongerExists(GuiceJamesServer server) throws Exception {
        ...
        mailboxProbe.deleteMessage(ImmutableList.of(teamMessage.getUid()), teamMailbox.inboxPath(), teamMailbox.owner());
        awaitVaultCount(vaultProbe, teamMailbox.self(), 1);
    ...
    void managerAndMemberShouldBehaveTheSameForTeamMailboxDmvRouting(GuiceJamesServer server) throws Exception {
        ...
        ComposedMessageId message1 = appendMessage(mailboxProbe, teamMailbox.inboxPath(), MESSAGE_CONTENT);
        ComposedMessageId message2 = appendMessage(mailboxProbe, teamMailbox.inboxPath(), SECOND_MESSAGE_CONTENT);

        // WHEN the manager deletes one message from the team mailbox
        mailboxProbe.deleteMessage(ImmutableList.of(message1.getUid()), teamMailbox.inboxPath(), andre);

        // AND the regular member deletes another message from the team mailbox
        mailboxProbe.deleteMessage(ImmutableList.of(message2.getUid()), teamMailbox.inboxPath(), bob);
    ...
    private void addMember(GuiceJamesServer server, Username username) {
        server.getProbe(TeamMailboxProbe.class).addMember(teamMailbox, username);
    }

    private void addManager(GuiceJamesServer server, Username username) {
        server.getProbe(TeamMailboxProbe.class).addManager(teamMailbox, username);
    }

    private void removeMember(GuiceJamesServer server, Username username) {
        server.getProbe(TeamMailboxProbe.class).removeMember(teamMailbox, username);
    }
