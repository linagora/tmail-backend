...
class DefaultTmailGroupRegistrationHandler implements TmailGroupRegistrationHandler {
    private static final Logger LOGGER = LoggerFactory.getLogger(DefaultTmailGroupRegistrationHandler.class);
    ...
    DefaultTmailGroupRegistrationHandler(NamingStrategy namingStrategy, EventSerializer eventSerializer,
                                         ReactorRabbitMQChannelPool channelPool, Sender sender,
                                         ReceiverProvider receiverProvider, EventDeadLetters eventDeadLetters,
        ...
        this.queueName = namingStrategy.workQueue(TmailGroupRegistrationHandler.GROUP);
        this.scheduler = Schedulers.newBoundedElastic(configurations.eventBusConfiguration().maxConcurrency(),
    ...
    public Mono<Void> reDeliver(Group group, Event event) {
        return retrieveGroupRegistration(group).reDeliver(event);
    }
    ...
    private GroupRegistration retrieveGroupRegistration(Group group) {
    ...
    private Disposable consumeWorkQueue() {
        return Flux.using(
                ...
                receiver -> receiver.consumeManualAck(queueName.asString(),
                    new ConsumeOptions().qos(configurations.eventBusConfiguration().maxConcurrency())),
                Receiver::close)
    ...
    private Mono<Void> deliver(AcknowledgableDelivery acknowledgableDelivery) {
        ...
        return deserializeEvents(eventAsBytes)
            ...
            .then()
            .onErrorResume(e -> {
                LOGGER.error("Unable to process delivery for group {}", TmailGroupRegistrationHandler.GROUP, e);
