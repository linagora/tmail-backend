...
/**
 * Like CalDavCollect but does not auto-import new events into the user's calendar.
 *
...
public class RestrictiveCalDavCollect extends GenericMailet {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(RestrictiveCalDavCollect.class);
    private static final String AUDIT_TRAIL_PROTOCOL = "mailetcontainer";
    private static final String AUDIT_TRAIL_ACTION = "RestrictiveCalDavCollect";
    private static final Class<Map<String, AttributeValue<byte[]>>> MAP_STRING_JSON_BYTES_CLASS =
        (Class<Map<String, AttributeValue<byte[]>>>) (Object) Map.class;
    ...
    private void handleCalendarInMail(byte[] json, Mail mail) {
        ...
        if (!icalContent.isEmpty() && !recipient.isEmpty()) {
            try {
                ...
                synchronizeIfEligible(mailAddress, calendar, json, senderAsString, mail);
            } catch (Exception e) {
    ...
    private static Optional<MailAddress> sentBy(Property calendarUser) {
        return calendarUser.getParameter(Parameter.SENT_BY)
            ...
            .flatMap(RestrictiveCalDavCollect::toCalendarUserMailAddress);
    }
    ...
    private void synchronizeIfEligible(MailAddress mailAddress, Calendar calendar, byte[] json, String sender, Mail mail) {
        if (isReply(calendar)) {
            ...
            if (!isExplicitAttendee(mailAddress, calendar)) {
                davUserProvider.provide(Username.of(mailAddress.asString()))
                    .flatMap(davUser -> synchronizeWithDavServer(json, davUser)
                        .then(auditCollectedEvent(mail, mailAddress, sender, calendar)))
                    .block();
            }
        } else {
            davUserProvider.provide(Username.of(mailAddress.asString()))
                .flatMap(davUser -> syncIfEventExists(json, calendar, davUser, mailAddress, sender, mail))
                .block();
        }
    }

    private Mono<Void> syncIfEventExists(byte[] json, Calendar calendar, DavUser davUser, MailAddress recipient, String sender, Mail mail) {
        ...
        return davClient.caldav(davUser.username()).getCalendarObject(davUser, eventUid)
            .flatMap(existing -> synchronizeWithDavServer(json, davUser)
                .then(auditCollectedEvent(mail, recipient, sender, calendar)));
    }
    ...
    private Calendar parseICalString(String icsContent) throws Exception {
        return new CalendarBuilder().build(new StringReader(icsContent));
    }
