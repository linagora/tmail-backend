    ...
    private void handleCalendarInMail(Calendar calendar, Mail mail) {
        try {
            mail.getMaybeSender().asOptional()
                .ifPresent(sender ->
                    davUserProvider.provide(Username.of(sender.asString()))
                        .flatMap(davUser -> davClient.getCalendarObject(davUser, getEventUid(calendar))
            ...
            LOGGER.error("Error while handling calendar in mail {}", mail.getName(), e);
        }
    }

    private Mono<Void> updateDavCalendar(DavCalendarObject davCalendarObject, Calendar calendar, DavUser davUser) {
        // TODO Rewrite this method to update the calendar event
