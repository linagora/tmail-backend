...
class AiBotSuggestionMethod @Inject()(val aiBotService: AIRedactionalHelper,
  ...
  override def doProcess(capabilities: Set[CapabilityIdentifier], invocation: InvocationWithContext, mailboxSession: MailboxSession, request: AiBotSuggestReplyRequest): Publisher[InvocationWithContext] = {
    val emailContentAsString: String = GetEmail(mailboxSession, request)

    aiBotService.suggestContent(request.userInput, Optional.ofNullable(emailContentAsString))
      .map(result => AiBotSuggestReplyResponse.from(request.accountId, result))
  ...
  private def extractEmailContent(messagesPublisher: Publisher[MessageResult]): String = {
    val flux = Flux.from(messagesPublisher)

    flux.collectList().map { messageResults =>
      messageResults.asScala.map { messageResult =>
        val inputStream = messageResult.getFullContent.getInputStream
        try {
