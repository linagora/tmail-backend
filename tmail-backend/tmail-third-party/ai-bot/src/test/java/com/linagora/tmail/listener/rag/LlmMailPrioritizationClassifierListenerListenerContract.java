...
public interface LlmMailPrioritizationClassifierListenerListenerContract {
    ...
    default void urgentEmailShouldBeTaggedNeedsAction() throws Exception {
        ...
        MessageId messageId = aliceInbox().appendMessage(MessageManager.AppendCommand.from(Message.Builder.of()
    ...
    // TODO more tests:
    // should not classify the mail if COPIED to INBOX
    // should not classify the mail if ADDED to another mailbox
    // should not classify the mail if the user is not in the recipient list
}
