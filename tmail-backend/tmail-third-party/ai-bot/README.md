...
The `LlmMailPrioritizationClassifierListener` is a mailbox listener that analyzes newly delivered emails and determines the appropriate user and system labels for it. Once a label matches the listener automatically  adds the `Label-ID` keyword to the message. This allows email clients to easily classify their emails.

The objective is to automatically categorize incoming emails using a flexible labeling system powered by a Large Language Model (LLM).

Given:

* A list of labels with descriptions
* The content of an email

The LLM determines which labels are relevant and attaches them to the email keywords.
The LLM used for classification is configurable via the ai.properties file.

### Label Types
The system supports two categories of labels:

1. User Labels:
   User labels are the mailbox labels: Defined and managed by end users, Fully customizable and Used for personal organization.

Examples: `AI`, `newsletter`, `finance`
2. System Labels:
   System labels are defined by system administrators ,not editable by users , used for standardized behaviors

Current system labels:

`action-needed` : Indicates that the email requires user attention
...
- **labels suggested**: List of labels suggested by the LLM
...
2. **Plug the extension in your TMail application**
      ...
      - Declare the plugin in the `extensions.properties` file
          ```bash
          guice.extension.module=com.linagora.tmail.common.AIBaseModule,com.linagora.tmail.jmap.aibot.AiMethodModule
          ```
