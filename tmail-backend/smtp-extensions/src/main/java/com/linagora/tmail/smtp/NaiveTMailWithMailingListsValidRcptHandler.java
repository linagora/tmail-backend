...
public class NaiveTMailWithMailingListsValidRcptHandler extends ValidRcptHandler {
    private final TeamMailboxRepository teamMailboxRepository;

    @Inject
    public NaiveTMailWithMailingListsValidRcptHandler(UsersRepository users,
    ...
    public boolean isValidRecipient(SMTPSession session, MailAddress recipient) throws UsersRepositoryException, RecipientRewriteTableException {
        return super.isValidRecipient(session, recipient)
            ...
            || mailingListHeuristic(session, recipient);
    }
    ...
    private boolean mailingListHeuristic(SMTPSession session, MailAddress recipient) {
        try {
            return isLocalDomain(session, recipient.getDomain())
                && recipient.getDomain().asString().startsWith("lists.");
