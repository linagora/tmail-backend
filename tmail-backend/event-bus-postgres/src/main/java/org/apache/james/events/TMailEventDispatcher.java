...
import static org.apache.james.events.RabbitMQAndPostgresEventBus.EVENT_BUS_ID;
...
public class TMailEventDispatcher {
    ...
    private Mono<Void> remoteDispatch(String eventAsJson, Collection<RoutingKey> routingKeys) {
        ...
        return Flux.fromIterable(routingKeys)
            .flatMap(routingKey -> getTargetChannels(routingKey)
                .flatMap(channel -> publishNotification(eventAsJson, routingKey, channel))
                .then())
            .then();
    }

    private Mono<Long> publishNotification(String eventAsJson, RoutingKey routingKey, String channel) {
        return Mono.usingWhen(postgresExecutor.connectionFactory().getConnection(Optional.empty()),
            connection -> {
                String statement = "NOTIFY " + quoteChannel(channel) + ", '" + KeyChannelMessage.from(eventBusId, routingKey, eventAsJson).serialize() + "'";
                return Mono.from(connection.createStatement(statement)
                        .execute())
    ...
    private Flux<String> getTargetChannels(RoutingKey routingKey) {
        return getChannels(routingKey);
    }

    private Flux<String> getChannels(RoutingKey routingKey) {
        return postgresExecutor.executeRows(dslContext -> Flux.from(dslContext.select(CHANNEL)
                .from(TABLE_NAME)
                .where(ROUTING_KEY.equal(routingKey.asString()))))
            .map(record -> record.get(CHANNEL));
    }
